    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        bottom: TabBar(
          ...
          tabs: const [
            Tab(icon: Icon(Icons.storefront_outlined), text: 'Jual'),
            Tab(
...
// ══════════════════════════════════════════════════════════════
// TAB 1 — Jual Sparepart (seluruh logic dipindah ke sini,
//          TANPA Scaffold karena sudah ada di SparepartScreen)
// ══════════════════════════════════════════════════════════════
class _JualSparepartTab extends StatefulWidget {
  const _JualSparepartTab();

  @override
  State<_JualSparepartTab> createState() => _JualSparepartTabState();
}

class _JualSparepartTabState extends State<_JualSparepartTab> {
  // ── State ──────────────────────────────────────────────────
  ...
  // ── BUILD — tidak pakai Scaffold ───────────────────────────
  @override
  Widget build(BuildContext context) {
    return Column(
      children: [
        // ── Mini-AppBar dalam tab: badge keranjang / judul ──
        _buildTabToolbar(),
        // ── Konten utama ────────────────────────────────────
        Expanded(
          child: _showKeranjang ? _buildKeranjang() : _buildCariSparepart(),
        ),
        // ── Tombol Bayar di bawah ────────────────────────────
        if (_showKeranjang && _keranjang.isNotEmpty) _buildBottomBayar(),
      ],
    );
  }
      ...
      child: Row(
        children: [
          if (_showKeranjang) ...[
            ...
            Text(
              'Keranjang ($_totalItem item)',
              style: const TextStyle(fontWeight: FontWeight.w600, fontSize: 15),
            ),
          ] else
            const Expanded(
              child: Text(
                'Cari & tambah sparepart',
                  ...
                  ...widget.keranjang.map((k) => Padding(
                        ...
                        child: Row(
                          ...
                          children: [
                            ...
                            Text(FormatHelper.currency(k.subtotal),
                                style: const TextStyle(fontSize: 13)),
                          ],
                        ),
                      )),
